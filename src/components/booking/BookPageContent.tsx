...
interface BookPageContentProps {
  /** Base path for redirects (e.g. "" for customer, "/staff" for staff). Back goes to basePath or "/", submit goes to basePath/booking/[code]. */
  basePath?: string;
}

export function BookPageContent({ basePath = "" }: BookPageContentProps) {
  ...
  useEffect(() => {
    ...
    setSelection(s);
    setChecked(true);
    if (!s) router.replace(homePath);
  }, [searchParams, router, homePath]);
  ...
  const handleSubmit = async (data: BookingFormValues) => {
    ...
    try {
      ...
      router.push(`${basePath}/booking/${booking.bookingCode}`);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-lg px-4 py-8">
        <BackButton href={homePath} className="mb-6" />
