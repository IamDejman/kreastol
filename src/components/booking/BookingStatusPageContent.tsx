...
interface BookingStatusPageContentProps {
  /** Base path for "Done" and not-found redirect (e.g. "" for customer, "/staff" for staff). */
  basePath?: string;
}

export function BookingStatusPageContent({ basePath = "" }: BookingStatusPageContentProps) {
  ...
  useEffect(() => {
    ...
    const loadBooking = async () => {
      ...
      setBooking(b);
      setChecked(true);
      if (!b) router.replace(homePath);
    };
    loadBooking();
  }, [code, router, homePath]);
  ...
  const doneUrl = (() => {
    ...
    return dateToFocus
      ? `${homePath}?focusDate=${encodeURIComponent(dateToFocus)}#book`
      : `${homePath}#book`;
  })();
